...
export const ProductCard: React.FC<ProductCardProps> = React.memo(({
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] w-full mb-3 bg-white dark:bg-[#1A1510] rounded-xl overflow-hidden flex items-center justify-center border border-gray-50 dark:border-gold-900/50">
        <img
        ...
        {badge && !hasPromoRibbon && (
          <div className="absolute top-0 left-0 z-10 w-full flex justify-start pointer-events-none">
            <span className={`px-2.5 py-1 text-[10px] sm:text-xs font-bold tracking-wide text-white rounded-br-xl shadow-sm ${badge === 'Sale' ? 'bg-gradient-to-r from-orange-500 to-gold-500' : 'bg-gradient-to-r from-teal-500 to-emerald-500'
              }`}>
        ...
        {hasPromoRibbon && (
          <div className="absolute top-0 left-0 z-10 w-full flex justify-start pointer-events-none">
            <span className="px-2.5 py-1 text-[10px] sm:text-xs font-bold text-white rounded-br-xl shadow-lg bg-gradient-to-r from-gold-600/90 via-purple-600/90 to-pink-500/90 backdrop-blur-md border-b border-r border-white/20">
              {promotionLabel || '⭐ مميز'}
            </span>
          </div>
        )}
        ...
        {hasDiscount && (
          <div className="absolute bottom-2 left-2 z-10 pointer-events-none">
            <span className="px-2 py-0.5 text-[10px] sm:text-[11px] font-bold text-white bg-gradient-to-r from-orange-500 to-gold-500 rounded-md shadow-md">
              -{discountPercentage}%
            </span>
          </div>
        )}
      ...
      <div className="flex flex-col flex-grow">
        ...
        <h3
          className="text-[13px] sm:text-sm font-medium text-gray-900 dark:text-slate-100 line-clamp-2 leading-snug mb-1.5 group-hover:text-gold-700 dark:group-hover:text-gold-400 transition-colors"
          title={title}
        >
          {title}
        </h3>
        ...
        <div className="flex items-center gap-1.5 mb-2.5">
                ...
                <div key={i} className="relative w-[12px] h-[12px] sm:w-[14px] sm:h-[14px]">
                  ...
                  {(isFilled || isHalf) && (
                    <div className={`absolute top-0 left-0 overflow-hidden ${isHalf ? 'w-1/2' : 'w-full'}`}>
                      <Star className="w-full h-full text-gold-400 fill-gold-400" />
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex items-center gap-1">
            <span className="text-[10px] sm:text-[11px] font-bold text-slate-700 dark:text-gold-400">
        ...
        <div className="mt-auto flex items-end justify-between relative pt-2">
          ...
          <button
            ...
            className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-full bg-blue-50 dark:bg-gold-900/40 text-blue-600 dark:text-gold-400 hover:bg-blue-600 dark:hover:bg-gold-600 hover:text-white transition-all duration-200 active:scale-90 flex-shrink-0 shadow-sm"
          >
