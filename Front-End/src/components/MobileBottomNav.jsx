        ...
        {tabs.map(({ to, icon: Icon, label, id, badge }) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span className="absolute top-0 inset-x-3 h-0.5 bg-gold-500 rounded-b-full" />
              )}
              ...
              <span className={`text-[10px] font-semibold leading-none ${isActive ? 'text-gold-600 dark:text-gold-400' : ''}`}>
                {label}
              </span>
            </Link>
          );
