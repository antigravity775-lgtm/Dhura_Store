...
const HeroSection = React.memo(() => {
  ...
  const getAlignClasses = () => {
    switch (banner.textAlign) {
      case 'left': return { text: 'text-left lg:text-left', items: 'items-end', textGradient: 'bg-gradient-to-r from-gold-400 to-gold-200' };
      case 'center': return { text: 'text-center lg:text-center', items: 'items-center', textGradient: 'bg-gradient-to-r from-gold-400 via-gold-200 to-gold-400' };
      default: return { text: 'text-right lg:text-right', items: 'items-start', textGradient: 'bg-gradient-to-l from-gold-400 to-gold-200' };
    }
  };
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10 sm:py-12 lg:py-14 mt-auto">
        <div className={`flex flex-col lg:flex-row items-end gap-8 lg:gap-12 ${banner?.textAlign === 'center' ? 'justify-center' : 'justify-between'}`}>
          ...
          >
            {/* Badge */}
            {badgeText && (
              <motion.div variants={itemVariants} className="inline-flex items-center gap-2 bg-gold-500/15 border border-gold-500/30 rounded-full px-4 py-1.5 mb-5 backdrop-blur-sm shadow-lg shadow-black/20">
                <Sparkles className="w-3.5 h-3.5 text-gold-400" />
                <span className="text-xs sm:text-sm font-bold text-gold-300 tracking-wide">{badgeText}</span>
              </motion.div>
            )}
            ...
            <motion.button
              ...
              id="hero-cta-btn"
              className="inline-flex items-center gap-2.5 px-8 py-4 bg-gradient-to-l from-gold-500 to-gold-400 text-white font-bold rounded-2xl shadow-xl shadow-gold-900/40 hover:from-gold-400 hover:to-gold-300 transition-all duration-300 text-lg"
            >
              <span>{ctaText}</span>
