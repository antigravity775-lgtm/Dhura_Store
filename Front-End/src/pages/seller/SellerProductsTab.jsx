    ...
    <>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
          ...
          <button onClick={() => setShowAddForm(true)} className="px-6 py-2.5 bg-gold-600 text-white font-bold rounded-xl hover:bg-gold-500">إضافة منتج</button>
        </div>
      ) : (
            ...
            .map(product => (
              ...
              >
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => openEditModal(product)} className="p-2 text-slate-300 dark:text-slate-500 hover:text-gold-600 dark:hover:text-gold-400 hover:bg-gold-50 dark:hover:bg-gold-900/30 rounded-xl transition-all" title="تعديل المنتج"><Edit3 className="w-4 h-4" /></button>
                  <button
                    onClick={() => handleToggleVisibility(product.id)}
                    disabled={togglingIds.has(product.id)}
                    className={`p-2 rounded-xl transition-all disabled:opacity-50 ${product.isHidden ? 'text-green-400 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30' : 'text-gold-400 hover:text-gold-600 hover:bg-gold-50 dark:hover:bg-gold-900/30'}`}
