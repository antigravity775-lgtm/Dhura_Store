    ...
    return (
      <Layout>
        <div className="max-w-2xl mx-auto px-4 py-24 text-center">
          ...
          <h2 className="text-3xl font-extrabold text-slate-900 mb-3">تم إنشاء الطلب بنجاح! 🎉</h2>
          <p className="text-slate-500 mb-8 text-lg">شكراً لك! سيتم التواصل معك قريباً لتأكيد الطلب.</p>
    ...
    return (
      <Layout>
        <div className="max-w-2xl mx-auto px-4 py-24 text-center">
          ...
          <h2 className="text-2xl font-extrabold text-slate-800 mb-2">سلة التسوق فارغة</h2>
          <p className="text-slate-500 mb-8">لم تقم بإضافة أي منتجات بعد. ابدأ بتصفح المنتجات!</p>
          <Link
            to="/"
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <AnimatePresence>
              {items.map(item => (
                <motion.div
                  ...
                  className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 flex gap-4 items-center shadow-sm hover:shadow-md transition-shadow"
                >
                  {/* صورة المنتج */}
                  <Link to={`/product/${item.productId}`} className="flex-shrink-0">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden bg-slate-100 border border-slate-200">
                      <img
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={`/product/${item.productId}`}>
                      <h3 className="font-bold text-slate-900 text-sm sm:text-base truncate hover:text-indigo-600 transition-colors">
                        {item.title}
                      </h3>
                    </Link>
          ...
          <div className="lg:w-[340px]">
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm sticky top-24">
              <h3 className="text-lg font-bold text-slate-900 mb-5">ملخص الطلب</h3>

              <div className="space-y-3 mb-5 text-sm">
                <div className="flex justify-between text-slate-500">
                  <span>المنتجات ({items.length})</span>
                  <span className="font-semibold text-slate-800">{cartTotal.toLocaleString('en-US')}</span>
                </div>
                <div className="flex justify-between text-slate-500">
                  <span>التوصيل</span>
                ...
                <div className="flex justify-between">
                  <span className="font-bold text-slate-900 text-base">الإجمالي</span>
                  <span className="font-extrabold text-indigo-600 text-lg">{cartTotal.toLocaleString('en-US')}</span>
                </div>
              </div>
